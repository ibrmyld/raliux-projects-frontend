...
const useSEO = ({
  ...
  useEffect(() => {
    // Site title
    const siteTitle = 'RailuX - AI-Powered Blog Platform';
    ...
    createMeta('keywords', keywords.join(', '));
          createMeta('author', author || 'RailuX Team');
    ...
    if (image) {
      ...
      createMeta('og:image:alt', title || 'Raliux Image', true);
    ...
    createMeta('twitter:site', '@Raliux');
    createMeta('twitter:creator', '@Raliux');
    
    if (image) {
      createMeta('twitter:image', image);
      createMeta('twitter:image:alt', title || 'Raliux Image');
    }
