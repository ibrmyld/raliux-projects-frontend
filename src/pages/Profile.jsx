...
const Profile = () => {
  ...
  const handleAvatarUpload = async (event) => {
    ...
    if (!allowedTypes.includes(file.type)) {
      setError('Only JPEG, PNG, GIF and WebP formats are supported.');
      return;
    }
    ...
    try {
      ...
      const response = await fetch(createApiUrl('/api/auth/upload-avatar'), {
        method: 'POST',
        headers: {
          'Authorization': `Bearer ${localStorage.getItem('token')}`
        },
      ...
      if (response.ok) {
        // Avatar URL'ini doğru şekilde güncelle
        const avatarUrl = data.avatar_url || data.avatar;
        const fullAvatarUrl = avatarUrl.startsWith('http') 
          ? avatarUrl 
          : `${import.meta.env.VITE_API_URL || 'https://raliux-backend.up.railway.app'}${avatarUrl}`;
        
        // AuthContext'i güncelle
        updateProfile({ avatar: fullAvatarUrl });
        setSuccess('Profile picture updated successfully!');
      } else {
        setError(data.detail || 'Error uploading photo.');
      }
    } catch (error) {
      ...
      <div className="min-h-screen flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-gray-600 dark:text-gray-400">
            You need to log in to view the profile page.
          </p>
        </div>
      </div>
    );
